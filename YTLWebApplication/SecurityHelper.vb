...
Public Class SecurityHelper
    ...
    Public Shared Function ContainsDangerousPatterns(input As String) As Boolean
        ...
        'Dim dangerousPatterns() As String = {
        '    "<\s*script\b", "</\s*script\s*>", "javascript\s*:", "vbscript\s*:",
        '    "on\w+\s*=", "eval\s*\(", "expression\s*\(", "url\s*\(", "import\s*\(",
        '    "document\s*\.", "window\s*\.", "alert\s*\(", "confirm\s*\(", "prompt\s*\(",
        '    "setTimeout\s*\(", "setInterval\s*\(",
        '    "union\s+select", "insert\s+into", "update\s+\w+\s+set", "delete\s+from",
        '    "drop\s+table", "create\s+table", "alter\s+table", "exec\s+xp_",
        '    "sp_oacreate", "sp_oamethod", "openrowset", "opendatasource",
        '    "xp_cmdshell", "bulk\s+insert", "truncate\s+table",
        '    "--", "/\*", "\*/"
        '}
